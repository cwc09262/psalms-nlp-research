...
def load_data():
    ...
    try:
        df_scored = pd.read_csv(OUTPUT_CSV)
        print(f"Loaded existing scored file: {OUTPUT_CSV}")
    except FileNotFoundError:
        df_scored = pd.DataFrame(columns=["Query", "Verse", "Psalm Num", "HumanScore"])
        print("No existing scored file found. Starting fresh.")
    ...
    if "HumanScore" not in df_input.columns:
        df_input["HumanScore"] = pd.NA

    # Check for new data not in the scored dataframe
    new_data = df_input[~df_input['Query'].isin(df_scored['Query'])]
    
    if not new_data.empty:
        print(f"Found {len(new_data)} new rows to score.")
        df_combined = pd.concat([df_scored, new_data], ignore_index=True)
        return df_combined
    else:
...
def show_row(df, idx, remaining, percent):
    row = df.loc[idx]
    print("\n======================================")
    print(f"UNSCORED PSALMS LEFT: {remaining}, ({percent:.2%})")
    print("QUERY:")
    print(row["Query"])
    print("\nRESULT:")
    print(row["Verse"])
